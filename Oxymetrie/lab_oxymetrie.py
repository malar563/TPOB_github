...
df = df[df["Time"] >= df["Time"].iloc[-1] - 26] # Garder les 25 dernières secondes
# df = df[(df["Time"] >= 11) & (df["Time"] <= 37)] # Garder entre les secondes 11 à 37
...
plt.plot(time, colors[0], color=plot_color[0], label="Signal infrarouge", linestyle="solid")
plt.plot(time, colors[1], color=plot_color[0], label="Signal rouge", linestyle="dashed")
...
plt.ylabel("Intensité normalisée [-]", fontsize=14)
plt.xlabel("Temps [s]", fontsize=14)
plt.figtext(0.2, 0.4, f"BPM = {bpm:.0f}±{i_bpm:.0f}\nSpO2 = ({100*spo2:.0f}±{100*i_spo2:.0f})%", fontsize=14)
plt.show()
