...
if __name__=='__main__':
    ...
    for t in types:
        ...
        # moyenner les images en les additionnant (le bruit devrait augmenter beaucoup moins vite que les speckles)
        sum_images = cv2.add(images[0],images[1])
        for i in range(1,20):
            sum_images = cv2.add(sum_images,images[i])

        # calculating contrast of the image

        average = np.mean(sum_images)
        standardDeviation = np.std(sum_images)
        contrast = standardDeviation / average
        print(f'contrast {t} :', contrast)
        contrastes[t] = float(round(contrast,3))
